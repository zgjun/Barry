...
@interface OtherCarSeriesController ()<UITableViewDelegate,UITableViewDataSource>
...
@property (nonatomic,strong) OtherCarDetailController *otherCarDetailVc;

@end

@implementation OtherCarSeriesController
...
- (void)getData {
    ...
    __weak OtherCarSeriesController *myself = self;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.indicator isEqualToString:@"CarType"]) {
        //跳转到下一个控制器
        OtherCarDetailController *otherCarDetailVc = [[OtherCarDetailController alloc]initWithCarSeriesId:dictList[@"id"] carSeriesName:dictList[@"name"]];
        self.otherCarDetailVc = otherCarDetailVc;
        [self.navigationController pushViewController:otherCarDetailVc animated:YES];
    } else {
        //通知代理
        if ([self.delegate respondsToSelector:@selector(otherCarSeriesSelected:)]) {
            [self.delegate otherCarSeriesSelected:seriesDict];
            //跳转
