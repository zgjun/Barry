...
@interface OtherLowPriceController ()<UITableViewDataSource,UITableViewDelegate,OtherBrandDelegate,OtherCarTypeDelegate,CityChooseControllerDelegate,UIGestureRecognizerDelegate,UITextFieldDelegate>
...
//@property (nonatomic,weak) UIButton *verifiCodeBtn;

@property (nonatomic,weak) UITextField *phoneField;

@property (nonatomic,weak) UITextField *nameField;

//@property (nonatomic,weak) UITextField *verifiCodeField;
...
@property (nonatomic,strong) OtherBrandController *otherChooseVc;

@property (nonatomic,strong) OtherCarTypeController *otherCarTypeVc;
@end

@implementation OtherLowPriceController
...
- (void)viewDidLoad {
    ...
    //创建talbeView
    UITableView *tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 64, ScreenWidth, ScreenHeight - 100 - 64)];
...
- (void)commitBtnClick {
    ...
    NSString *commitUrl = [NSString stringWithFormat:@"http://dealer.chexun.com/API/FenFaXunJiaHandler3.ashx?phone=%@&seriesId=%@&modelId=%@&userName=%@&cityId=%@&Source=2",phone,self.carSeriesId,self.carTypeId,self.nameField.text,self.cityId];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        OtherBrandController *otherChooseVc = [[OtherBrandController alloc]init];
        self.otherChooseVc = otherChooseVc;
        otherChooseVc.delegate = self;
        ...
        OtherCarTypeController *otherCarTypeVc = [[OtherCarTypeController alloc]initWithCarSeriesId:self.carSeriesId];
        self.otherCarTypeVc = otherCarTypeVc;
        otherCarTypeVc.delegate = self;
        [self.navigationController pushViewController:otherCarTypeVc animated:YES];
    }
}
...
- (void)otherBrandSelected:(NSDictionary *)carSeriesDict {
    self.carSeriesName = carSeriesDict[@"name"];
    self.carSeriesId = carSeriesDict[@"seriesId"];
    
    [self.tableView reloadData];
}

- (void)otherCarTypeSelected:(NSDictionary *)carTypeDict {
    self.carTypeName = carTypeDict[@"name"];
    self.carTypeId = carTypeDict[@"id"];
    [self.tableView reloadData];
}
